...
**Produto:** ENEM Express, R$ 97 em pagamento único, com todas as funcionalidades inteligentes de IA até 15/11/2026 (fim do dia em São Paulo), último dia da aplicação regular conforme o [Inep](https://www.gov.br/inep/pt-br/areas-de-atuacao/avaliacao-e-exames-educacionais/enem/orientacoes/cronograma). Catálogo fixo em `lib/billing/enem-express.ts`. O pagamento confirmado ativa o plano automaticamente, substituindo o plano atual, sem renovação. Mantém os limites técnicos existentes de 30 solicitações diárias e 5 uploads mensais; acesso a todas as funcionalidades não significa consumo ilimitado. A consulta de permissões desconsidera planos vencidos. Contas com histórico do ENEM Express sem plano vigente não retornam às cotas de IA do piloto.

**Ativação pendente:** aplicar a nova migration `20260905140000_asaas_refunds.sql`, atualizar os eventos do webhook e homologar o fluxo antes de definir `ASAAS_CHECKOUT_ENABLED=true`. A interface permanece com matrículas desativadas por padrão. Nenhuma credencial, migração remota ou configuração do Asaas é alterada automaticamente.


## Complemento do webhook e homologação

Além dos quatro eventos `CHECKOUT_*`, selecionar **PAYMENT_REFUNDED**, **PAYMENT_CHARGEBACK_REQUESTED** e **PAYMENT_CHARGEBACK_DISPUTE** no Asaas. Esses eventos suspendem o pedido e cancelam apenas a assinatura ligada a ele. A associação utiliza `payment.externalReference` (UUID do pedido) ou `payment.checkoutSession` quando disponível. Eventos sem associação ficam na inbox com `processed_at` nulo e exigem reconciliação operacional; verificar essa associação com o payload real na homologação. Estorno parcial e reversão favorável de disputa exigem análise manual; uma confirmação atrasada não reativa automaticamente um pedido revogado.

O login, a recuperação e os callbacks agora permanecem no domínio de origem. No Supabase, autorizar `https://www.notaalvo.com.br/**` e `https://app.notaalvo.com.br/**` nas Redirect URLs. As sessões permanecem separadas por host: ao acessar a área do aluno pode ser necessário entrar novamente com a mesma conta. O resultado consulta `/api/billing/status`, autenticado, sem confiar nos parâmetros de retorno do checkout.

Homologar: quiz → cadastro/login → checkout de R$ 97 → confirmação via webhook → plano ativo; repetir evento; enviar estorno associado → pedido revogado e plano cancelado; confirmar que um CHECKOUT_PAID atrasado não restabelece acesso. Testar também expiração, token inválido e indisponibilidade do banco. Usar sandbox para pagamentos simulados: não forjar confirmação de compra em produção. Configurações remotas, cobrança real e deploy não são executados pelos testes locais.
