...
export async function POST(request: Request) {
  ...
  if (body?.action === "seed_enem_pilot") {
    ...
    if (!exam) {
      const created = await access.admin.from("exams").insert({ institution: "INEP", state: "DF", role: "ENEM — treino autoral", exam_year: 2026, organizer: "Nota Alvo", source_url: "https://www.gov.br/inep/pt-br/areas-de-atuacao/avaliacao-e-exames-educacionais/enem", authorization_reference: "Questões autorais Nota Alvo; referência de competências no portal oficial do Inep", status: "published", slug: "enem-2026-autoral-rota", title: "ENEM 2026 — diagnóstico autoral Nota Alvo", metadata: { officialQuestions: false, purpose: "pilot_diagnostic" } }).select("id").single();
