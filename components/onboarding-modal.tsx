...
import { ENEM_FOCUS_AREAS, primaryInterestsForCareer, type EnemFocusArea } from "@/lib/domain/onboarding";
...
export function OnboardingModal({ open, onClose, onComplete }: Props) {
  ...
  function selectCareer(value: string) {
    setCareer(value);
    setInterests(primaryInterestsForCareer(value));
    if (value === "enem-2026") {
      setNotice("published");
      setExamDate("2026-11-08");
    } else {
      setEnemFocusArea("undecided");
      setTargetCourse("");
  ...
  function finish() {
    completeOnboarding({
      ...
      preferredPeriod,
      interests,
      enemFocusArea: isEnem ? enemFocusArea : null,
      targetCourse: isEnem ? targetCourse.trim() || null : null,
      targetInstitution: isEnem ? targetInstitution.trim() || null : null,
      preferredFormats,
