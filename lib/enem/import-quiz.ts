import { landingQuestions, parseQuizAttempt, evaluateQuiz } from "./landing-quiz";
...
export function importQuiz(state: RotaState, id: string, value: unknown, now = new Date()) {
  ...
  if (state.importedQuizId || !attempt || !evaluateQuiz(attempt.answers) || state.profile.onboardingCompleted && state.profile.career !== "enem-2026") return state;
  ...
  landingQuestions.forEach((q, index) => {
    next = recordAnswer(next, { ...q, id: `landing:${q.id}`, topicId: topics[index], axis: q.area, difficulty: "Fácil" }, attempt.answers[index]!, "diagnostic", new Date(now.getTime() + index));
  });
